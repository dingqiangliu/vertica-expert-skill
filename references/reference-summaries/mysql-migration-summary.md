...
Load [mysql-migration.md](../mysql-migration.md) section by section when:
...
How to load: `grep -n "^## \|^### " references/mysql-migration.md` → `Read offset=N limit=M` (load ONLY that section, NOT the entire file).
