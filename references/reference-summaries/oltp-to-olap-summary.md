...
Load [oltp-to-olap-rewrite-guide.md](../oltp-to-olap-rewrite-guide.md) section by section when:
...
How to load: `grep -n "^## \|^### " references/oltp-to-olap-rewrite-guide.md` → `Read offset=N limit=M` (load ONLY that section, NOT the entire file).
