...
Load [sqlserver-migration.md](../sqlserver-migration.md) section by section when:
...
How to load: `grep -n "^## \|^### " references/sqlserver-migration.md` → `Read offset=N limit=M` (load ONLY that section, NOT the entire file).
