...
| 3 | **ALWAYS use PERFORM** for DDL, DML, CALL, COMMIT, ROLLBACK, EXECUTE that are not on the right-hand side of an assignment | `syntax error, unexpected <keyword>, expecting := or <-` |
...
### Common Pitfalls
- SQL statement not in assignment or PERFORM position → `syntax error, unexpected <keyword>, expecting := or <-`
...
Every SQL statement produces a response in Vertica. PL/vSQL does not have a "standalone SQL statement" syntax — embedded SQL must appear either on the right-hand side of an assignment (`var := SQL`) or after PERFORM (`PERFORM SQL`). A SQL keyword in any other position is treated as an identifier, causing: `syntax error, unexpected <keyword>, expecting := or <-`. Use `PERFORM` when you do not need to capture the SQL result.
...
Load [stored-procedures-guide.md](../stored-procedures-guide.md) section by section when:
...
How to load: `grep -n "^## \|^### " references/stored-procedures-guide.md` → `Read offset=N limit=M` (load ONLY that section, NOT the entire file).
